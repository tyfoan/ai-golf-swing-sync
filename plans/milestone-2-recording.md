...
1. [x] CameraService with back camera recording
2. [x] CameraPreviewView (SwiftUI wrapper)
3. [x] Basic recording start/stop
4. [x] High frame rate configuration (120fps)

### Week 2: Real-Time Pose
1. [x] LivePoseDetector with frame callback
2. [x] PoseOverlayView skeleton rendering
3. [x] Performance optimization (skip frames if needed)
4. [ ] Test pose detection accuracy

### Week 3: Live Swing Detection
1. [x] LiveSwingDetector with ring buffer (also MLSwingDetector)
2. [x] Real-time velocity tracking
3. [x] Swing detection callbacks
4. [ ] Test detection latency

### Week 4: Recording Flow
1. [x] RecordingViewModel state machine
2. [x] CountdownView
3. [x] LiveRecordingView with pose overlay (RecordingView.swift)
4. [x] Swing detected notification UI (SwingAttemptCard)

### Week 5: PiP & Replay
1. [ ] Dual camera setup (if supported)
2. [ ] Front camera PiP view
3. [x] SwingReplayOverlay (SwingReplayView.swift)
